...
def search_all_records(familia):
    flag = True
    with (open(file_path, 'r', encoding="utf-8")) as _data:
        for line in _data:
            if familia.lower() in line.split(";")[0].lower():
                print(line.replace(";", " "), end="")
                flag = False
    if flag:
        print(f'Контакт {familia} не найден')


def import_new_record(new_contact_fio, new_contact_number):
...
def main():
    while True:
        ...
        if select == 1:
            ...
            search_all_records(surname)
        elif select == 3:
            fio = input("Введите ФИО через пробел: ")
            number = input("Введите номер телефона: ")
            import_new_record(fio, number)
        elif select == 4:
            # ....
            change_one_record()
        elif select == 5:
